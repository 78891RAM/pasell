...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<bool> changePass(String oldpassword, String passowrd) async {
    ...
    if (data['message'] == 'Password update sucessfully') {
      ...
      MotionToast.success(
              description: const Text("Password update sucessfully"))
          .show(context);
      return true;
    } else if (data['message'] == 'Password must be more that 6 charector') {
      // ignore: use_build_context_synchronously
      MotionToast.error(
              description: const Text("Password must be more that 6 charector"))
          .show(context);
      return false;
      ...
      MotionToast.error(
              description:
                  const Text("Old password and new password is too similar"))
          .show(context);
      return false;
    }
    // ignore: use_build_context_synchronously
    MotionToast.error(description: const Text("Old password not match"))
        .show(context);
    return false;
  }
      ...
      appBar: AppBar(
        ...
        title: const Center(
          child: Row(
            ...
            children: [
              Text("Pasell", style: TextStyle(fontSize: 19)),
            ],
          ),
        ),
      ),
                ...
                ? SingleChildScrollView(
                    child: Container(
                        ...
                        decoration: const BoxDecoration(
                            color: Color.fromARGB(223, 8, 59, 170)),
                        child: Center(
                          child: Column(
                            children: [
                              Padding(
